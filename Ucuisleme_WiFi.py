...
my_ip = '10.10.120.194'                                # IP kameranın adresi.
ret=1                                                  # Döngünün aktif kalması için. 
...
while True:
    req = urllib.request.urlopen('http://'+my_ip+':8080/shot.jpg')   # IP Adresi
    arr = np.asarray(bytearray(req.read()), dtype=np.uint8)          # Wi-Fi videodan gelen veri.
    frame = cv2.imdecode(arr, -1)                                    # Oldugu gibi yükle. 
